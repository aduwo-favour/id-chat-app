...
function showInAppNotification(payload) {
  ...
  // Respect per-chat mute
  const _mid = payload.data?.chatId;
  if (_mid && mutedChats.includes(_mid)) return;
...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    if (userDoc.exists()) {
      ...
      // --- Admin global settings: LIVE maintenance mode + announcement banner.
      // Re-evaluates instantly whenever an admin flips a setting (no refresh). ---
      const ANN_ID = 'global-announcement-bar';
      subscribeGlobalSettings(gSettings => {
        // Maintenance: kick non-admins the moment it's turned on
        ...
        // Announcement: add / update / remove the banner live
        const existing = document.getElementById(ANN_ID);
        const text = (gSettings.announcement || '').trim();
        if (text) {
